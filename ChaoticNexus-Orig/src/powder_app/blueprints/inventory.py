...
def adjust_inventory(powder_id: int):
    ...
    flash(f"Inventory adjusted for {powder['powder_color']}: {adjustment_type} {adjustment} kg", "success")
    return redirect(url_for("inventory.inventory_page"))
...
def inventory_history(powder_id: int):
    ...
    if not (is_admin() or has_perm("see_powders")):
        return redirect(url_for("auth.login", next=url_for("inventory.inventory_history", powder_id=powder_id)))
