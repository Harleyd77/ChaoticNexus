...
def jobs_kanban():
    ...
    for key, items in columns.items():
        items.sort(key=lambda job: (job.get("priority_rank", 99), job.get("_due_dt") or far_future, -(job.get("id") or 0)))
        for job in items:
...
def jobs_kanban_move():
    ...
    if current_dept == target_dept:
        ...
        card = _serialize_job_for_kanban(job_dict, target_dept in TIMER_ACTIVE_DEPARTMENTS, now, now + timedelta(hours=24))
...
def job_photo_delete(job_id: int, photo_id: int):
    ...
    if guard:
        return guard
    row = db_query_one("SELECT filename FROM job_photos WHERE id=? AND job_id=?", (photo_id, job_id))
    if row:
        fname = row["filename"]
        try:
...
def archive_job(job_id: int):
    ...
    db_execute("UPDATE jobs SET archived=1, archived_reason=?, on_screen=0 WHERE id=?", (reason, job_id))
    return redirect(request.referrer or url_for("jobs.jobs_board"))
...
def jobs_complete(job_id: int):
    if not is_admin():
        return redirect(url_for("login", next=url_for("jobs.jobs_board")))
    db_execute("UPDATE jobs SET on_screen=0, completed_at=? WHERE id=?", (datetime.now().isoformat(), job_id))
    return redirect(request.referrer or url_for("jobs.jobs_board"))
...
def job_workorder(job_id: int):
    ...
    raw_date = next((job.get(key) if isinstance(job, dict) else job[key] for key in ("date_in", "due_by", "created_at") if (job.get(key) if isinstance(job, dict) else job[key])), None)
    display_date = None
    if raw_date:
        parsed = None
    ...
    template_label = "Railing Intake Worksheet" if template_type == "job_worksheet_railing" else "Production Intake Worksheet"
