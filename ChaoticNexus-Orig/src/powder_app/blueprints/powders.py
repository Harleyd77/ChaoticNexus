...
from ..core.utils import fmt_ts, to_float, slugify
...
def powders_import():
    ...
    if not reader.fieldnames or "powder_color" not in [h.strip().lower() for h in reader.fieldnames if h]:
    ...
    try:
        for row in reader:
            ...
            if powder_id:
                existing = conn.execute("SELECT id FROM powders WHERE id=?", (powder_id,)).fetchone()
                if existing:
