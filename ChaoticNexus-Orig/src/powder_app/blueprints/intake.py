...
from ..core.customer_auth import can_customer_access_intake, get_current_customer, get_customer_contact_info, is_customer_logged_in
...
def intake_form():
    ...
    return render_template("intake_form.html", customer_info=customer_info, is_customer_logged_in=is_customer_logged_in())
...
def railing_intake():
    ...
    return render_template("RailingIntake.html", customer_info=customer_info, is_customer_logged_in=is_customer_logged_in())


@bp.route("/submit", methods=["POST"])
def submit_form():
    if not (is_admin() or has_perm("see_intake") or has_perm("see_railing") or can_customer_access_intake()):
        return redirect(url_for("login", next=url_for("intake_form")))
    ...
    for file in files:
        ...
        try:
            ...
            safe_base = secure_filename(os.path.splitext(original)[0] if original else "") or "upload"
