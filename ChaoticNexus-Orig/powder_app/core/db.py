...
def _migrate_sqlite_schema(connection: sqlite3.Connection) -> None:
    ...
    try:
        ...
        cursor.execute("UPDATE jobs SET intake_source='production' WHERE intake_source IS NULL")
    except Exception:
        pass
...
def init_db_postgres():  # pragma: no cover - requires Postgres
    ...
    has_cure = db.execute(
        "SELECT 1 FROM information_schema.columns WHERE table_name='powders' AND column_name='cure_schedule'"
    ).fetchone()
    if not has_cure:
