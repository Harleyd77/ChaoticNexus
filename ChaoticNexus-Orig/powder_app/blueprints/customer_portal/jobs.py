...
def job_detail(customer, job_id):
    ...
    return render_template(
        "customer_portal/job_detail.html", customer=customer, job=job, edit_history=edit_history
    )
...
def edit_job(customer, job_id):
    ...
    if request.method == "POST":
        ...
        if errors:
            for error in errors:
                flash(error, "error")
            return render_template("customer_portal/job_edit.html", customer=customer, job=job)
