...
def customer_detail(cust_id: int):
    ...
    contacts = db_query_all(
        "SELECT * FROM contacts WHERE customer_id=? ORDER BY id DESC", (cust_id,)
    )
    return render_template(
...
def admin_customer_account_detail(account_id):
    ...
    if not is_admin():
        return redirect(
            url_for("login", next=url_for("admin_customer_account_detail", account_id=account_id))
        )
...
def api_admin_update_customer_account(account_id):
    ...
    if updates:
        ...
        db_execute(f"UPDATE customer_accounts SET {', '.join(updates)} WHERE id = ?", params)
