...
def _ensure_sprayer_schema_sqlite() -> None:
    ...
    if "elapsed_seconds" not in sbj_cols:
        ...
        db.execute(
            "UPDATE spray_batch_jobs SET elapsed_seconds = COALESCE(time_min, 0) * 60.0 WHERE elapsed_seconds IS NULL OR (elapsed_seconds = 0 AND COALESCE(time_min, 0) > 0)"
        )
...
def _ensure_sprayer_schema_pg() -> None:
    ...
    db.execute(
        "ALTER TABLE spray_batch_jobs ADD COLUMN IF NOT EXISTS elapsed_seconds REAL DEFAULT 0"
    )
    db.execute("ALTER TABLE spray_batch_jobs ADD COLUMN IF NOT EXISTS running_since TEXT")
    db.execute(
        "UPDATE spray_batch_jobs SET elapsed_seconds = COALESCE(elapsed_seconds, COALESCE(time_min, 0) * 60.0) WHERE elapsed_seconds IS NULL OR (elapsed_seconds = 0 AND COALESCE(time_min, 0) > 0)"
    )
...
def sprayer_batch_detail(batch_id: int):
    ...
    if not batch:
        abort(404)
    powder = db.execute(
        "SELECT powder_color, manufacturer FROM powders WHERE id=?", (batch["powder_id"],)
    ).fetchone()
    job_rows = db.execute(
        """
    ...
    for row in job_rows:
        ...
        job["is_paused"] = (
            bool(job.get("start_ts")) and not job["is_running"] and not job["is_complete"]
        )
        attached.append(job)
...
def sprayer_batch_add_job(batch_id: int):
    ...
    for jid in job_ids:
        db.execute(
            "INSERT OR IGNORE INTO spray_batch_jobs (batch_id, job_id) VALUES (?,?)",
            (batch_id, jid),
        )
    db.commit()
    return redirect(url_for("sprayer.sprayer_batch_detail", batch_id=batch_id))
...
def sprayer_job_start(batch_id: int, job_id: int):
    ...
    db = get_db()
    db.execute(
        "INSERT OR IGNORE INTO spray_batch_jobs (batch_id, job_id) VALUES (?,?)", (batch_id, job_id)
    )
